...
export const paperDownloadOrder = pgTable(
  'paper_download_order',
  {
    ...
    title: text('title').notNull(),
    doi: text('doi'),
    amountFen: integer('amount_fen').notNull().default(100),
    status: varchar('status', {
      enum: ['requested', 'stored', 'debited', 'delivered', 'refund_pending', 'refunded', 'failed'],
    })
      .notNull()
      .default('requested'),
    ...
    debitIdempotencyKey: varchar('debit_idempotency_key', { length: 160 }).notNull(),
    refundIdempotencyKey: varchar('refund_idempotency_key', { length: 160 }).notNull(),
  ...
  (table) => [
    uniqueIndex('paperDownloadOrder_user_request_unique').on(table.userId, table.clientRequestId),
    uniqueIndex('paperDownloadOrder_debit_key_unique').on(table.debitIdempotencyKey),
    uniqueIndex('paperDownloadOrder_refund_key_unique').on(table.refundIdempotencyKey),
    index('paperDownloadOrder_user_created_idx').on(table.userId, table.createdAt),
    index('paperDownloadOrder_status_updated_idx').on(table.status, table.updatedAt),
  ],
);

export const scientificFigureJob = pgTable(
  'scientific_figure_job',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    userId: text('user_id')
      .notNull()
    ...
    stageLabels: jsonb('stage_labels').$type<string[]>().notNull(),
    width: integer('width').notNull(),
    height: integer('height').notNull(),
    status: varchar('status', { enum: ['queued', 'generating', 'completed', 'failed'] })
      .notNull()
      .default('queued'),
  ...
  (table) => [
    index('scientificFigureJob_user_created_idx').on(table.userId, table.createdAt),
    index('scientificFigureJob_status_updated_idx').on(table.status, table.updatedAt),
  ],
);
...
export const userRelations = relations(user, ({ many }) => ({
  ...
  paperDownloadOrders: many(paperDownloadOrder),
  scientificFigureJobs: many(scientificFigureJob),
}));
...
export const paperDownloadOrderRelations = relations(paperDownloadOrder, ({ one }) => ({
  user: one(user, {
    fields: [paperDownloadOrder.userId],
    references: [user.id],
  }),
}));

export const scientificFigureJobRelations = relations(scientificFigureJob, ({ one }) => ({
  user: one(user, {
    fields: [scientificFigureJob.userId],
    references: [user.id],
  }),
}));
...
export type PaperDownloadOrder = InferSelectModel<typeof paperDownloadOrder>;
export type ScientificFigureJob = InferSelectModel<typeof scientificFigureJob>;
